...
if (-not $SkipBuild) {
    ...
    ssh -i $KeyPath $ServerHost "cd $RemoteDir/deploy && docker compose --env-file .env.production -f docker-compose.prod.yml build api admin"
}

Write-Host "Restarting stack ..."
ssh -i $KeyPath $ServerHost "cd $RemoteDir/deploy && docker compose --env-file .env.production -f docker-compose.prod.yml up -d && docker compose --env-file .env.production -f docker-compose.prod.yml ps"
